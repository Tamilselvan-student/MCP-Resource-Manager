...
⚠️ They must change their password on first login.`,
    ...
    const hasRole = role && VALID_ROLES.includes(role as any);
...
async function executeGetUserInfo(
    ...
    try {
        ...
        const roleEmojiMap: { [key: string]: string } = {
            'owner': '🟣',
            'admin': '🔴',
            'editor': '🔵',
            'viewer': '⚪'
    ...
    try {
        ...
        const duplicateCheck = await pool.query(
            `SELECT data->>'name' as name 
             ...
             AND LOWER(data->>'category') = LOWER($2)
             LIMIT 1`,
            [resourceName, matchedNewCategory]
        );
...
async function executeUserHistory(command: ParsedCommand, executorId: string): Promise<CommandResult> {
    ...
    try {
        ...
        historyQuery.rows.forEach((record, index) => {
            ...
            if (record.reason) response += `   Reason: "${record.reason}"\n`;
            response += `\n`;
        });
