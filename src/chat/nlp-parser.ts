...
/**
 * COMPREHENSIVE CHAT PERSONA DEFINITION
 * 
 * This AI assistant is a friendly, helpful admin assistant with personality.
 * It handles everything from casual greetings to complex admin operations.
 * 
 * PERSONALITY:
 * - Friendly and approachable
 * - Uses retro cyberpunk language (emojis, cool tech terms)
 ...
 * 
 * CAPABILITIES:
 * 1. Greetings & Goodbyes (hi, hello, bye, see you)
 * 2. Gratitude & Compliments (thanks, good job, awesome)
 * 3. Help & Guidance (help, what can you do, how do I)
 * 4. User Management (show users, change roles, create/delete users)
 * 5. Resource Management (who has access, visibility, create/delete)
 * 6. System Information (stats, user details, system health)
 * 7. Casual Conversation (how are you, what's up, yo)
 * 8. Error Recovery (did you mean, suggestions, clarifications)
 * 
 * RESPONSE STYLE:
 * - Use emojis liberally (💬 👋 ✅ ❌ 🎉 💪 🔥 ⚡)
 * - Keep responses concise but friendly
 * - Offer follow-up suggestions
 * - Use retro language (cyber, matrix, system, protocol)
 */
...
export type IntentType =
    // Social Intents
    | 'greeting' | 'goodbye' | 'gratitude' | 'compliment' | 'casual_question' | 'small_talk'
    // Help Intents
    | 'help' | 'capabilities' | 'how_to'
    // User Management Intents
    | 'list_users' | 'find_user' | 'get_user_info' | 'change_role'
    | 'create_user' | 'delete_user'
    // Enhanced User Queries
    | 'user_creation_date' | 'user_full_details' | 'user_creator'
    | 'user_last_active' | 'user_history' | 'user_created_resources'
    ...
    // Filtered Lists
    | 'list_inactive_users' | 'list_active_users' | 'list_users_need_password_change'
    // System Information
    | 'system_stats' | 'who_am_i'
    // Fallback
    | 'unknown';
...
export const FALLBACK_RESPONSES = {
    user_not_found: (name: string, suggestions: string[]) => {
    ...
    resource_not_found: (name: string, suggestions: string[]) => {
        if (suggestions.length > 0) {
    ...
    permission_denied: (action: string, required_role: string) => {
...
export async function findUserByIdentifier(identifier: string): Promise<any> {
...
export async function findResourceByName(name: string): Promise<any> {
...
export function getRandomResponse(templates: string[]): string {
 ...
 */
function isResourceQuery(message: string): boolean {
 ...
 */
function isUserQuery(message: string): boolean {
    const lower = message.toLowerCase();
 ...
 */
function detectEntityType(message: string): 'resource' | 'user' | 'system' | 'unknown' {
    const lower = message.toLowerCase();
