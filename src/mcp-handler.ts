...
// ============================================
// INTERFACES - Resource-Agnostic Design
// ============================================

export interface MCPRequest {
    action: 'list' | 'read' | 'create' | 'update' | 'delete' | 'meta' | 'none';
    resourceType: string;  // Dynamic: files, appointments, projects, etc.
    resourceId?: string;
    data?: any;
    userId: string;
    filters?: Record<string, any>;  // For advanced queries
}
...
export class MCPHandler {
    ...
    private async handleList(request: MCPRequest): Promise<MCPResponse> {
        ...
        try {
            ...
            try {
                const roleResult = await this.dbPool.query(
    ...
    private async handleRead(request: MCPRequest): Promise<MCPResponse> {
    ...
    private async handleCreate(request: MCPRequest): Promise<MCPResponse> {
        ...
        try {
            const result: QueryResult = await this.dbPool.query(
                'INSERT INTO resources (resource_type, data, created_by) VALUES ($1, $2, $3) RETURNING *',
    ...
    private async handleUpdate(request: MCPRequest): Promise<MCPResponse> {
        ...
        try {
            const result: QueryResult = await this.dbPool.query(
                'UPDATE resources SET data = $1, updated_at = NOW() WHERE id = $2 AND resource_type = $3 RETURNING *',
                [JSON.stringify(data), resourceId, resourceType]
            );
    ...
    private async handleDelete(request: MCPRequest): Promise<MCPResponse> {
        const { resourceType, resourceId, userId } = request;
        ...
        try {
            const result: QueryResult = await this.dbPool.query(
                'DELETE FROM resources WHERE id = $1 AND resource_type = $2 RETURNING *',
                [resourceId, resourceType]
            );
