...
async function cleanupWildcards() {
    ...
    try {
        ...
        do {
            ...
            const wildcards = tuples.filter((t) => t.key.object.endsWith('_*'));
            tuplesToDelete = [...tuplesToDelete, ...wildcards.map((t) => t.key)]; // extract key
        ...
        if (tuplesToDelete.length > 0) {
            // Batch delete
            const deleteResponse = await fetch(`${process.env.FGA_API_URL}/stores/${process.env.FGA_STORE_ID}/write`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    deletes: {
