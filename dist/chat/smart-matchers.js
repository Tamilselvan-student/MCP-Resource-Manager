 ...
 */
export function matchCategory(input) {
    ...
    const partialMatch = VALID_CATEGORIES.find(c => c.toLowerCase().includes(lower) || lower.includes(c.toLowerCase()));
    if (partialMatch) {
        return partialMatch;
    }
    // Try keyword match
...
export function matchRole(input) {
    ...
    // Try partial match
    const partialMatch = VALID_ROLES.find(r => r.startsWith(lower) || lower.startsWith(r));
    if (partialMatch) {
        return partialMatch;
    }
    return null;
