    ...
    try {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (tuples.length > 0) {
                    const deletes = tuples.map((t) => t.key);
                    ...
                    if (deleteRes.ok)
                        console.log(`✅ Deleted ${tuples.length} orphaned tuples.`);
                    else
                        console.warn('⚠️ Failed to delete orphaned tuples:', await deleteRes.text());
                }
                else {
    ...
    try {
        ...
        try {
            ...
            if (writes.length > 0 || deletes.length > 0) {
                ...
                if (!swapRes.ok) {
                    console.error('❌ Failed to swap groups:', await swapRes.text());
                }
            }
        }
        catch (e) {
